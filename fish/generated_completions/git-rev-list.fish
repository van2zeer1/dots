# git-rev-list
# Autogenerated from man page /bedrock/cross/man/man1/git-rev-list.1
...
complete -c git-rev-list -l first-parent -d 'Follow only the first parent commit upon seeing a merge commit'
...
complete -c git-rev-list -l disk-usage -d 'Suppress normal output; instead, print the sum of the bytes used for on-disk …'
...
complete -c git-rev-list -l encoding -d 'The commit objects record the encoding used for the log message in their enco…'
