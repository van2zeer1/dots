# git-diff-tree
# Autogenerated from man page /bedrock/cross/man/man1/git-diff-tree.1
complete -c git-diff-tree -s p -s u -l patch -d 'Generate patch (see section on generating patches)'
...
complete -c git-diff-tree -o 'l<num>' -d 'The -M and -C options require O(n^2) processing time where n is the number of…'
...
complete -c git-diff-tree -l encoding -d 'The commit objects record the encoding used for the log message in their enco…'
