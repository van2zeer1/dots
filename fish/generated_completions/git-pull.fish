# git-pull
# Autogenerated from man page /bedrock/cross/man/man1/git-pull.1
...
complete -c git-pull -l ff -l no-ff -l ff-only -d 'Specifies how a merge is handled when the merged-in history is already a desc…'
...
complete -c git-pull -l no-verify -d 'This option bypasses the pre-merge and commit-msg hooks'
...
complete -c git-pull -l autostash -l no-autostash -d 'Automatically create a temporary stash entry before the operation begins, and…'
...
complete -c git-pull -l no-rebase -d 'Override earlier --rebase'
...
complete -c git-pull -l preserve-merges -d 'option passed to git rebase so that locally created merge commits will not be…'
...
complete -c git-pull -l patience -d 'diff-algorithm=[patience|minimal|histogram|myers]'
complete -c git-pull -l diff-algorithm -d 'ignore-space-change, ignore-all-space, ignore-space-at-eol, ignore-cr-at-eol'
...
complete -c git-pull -l no-renames -d 'find-renames[=<n>]'
