# git-diff
# Autogenerated from man page /bedrock/cross/man/man1/git-diff.1
complete -c git-diff -s p -s u -l patch -d 'Generate patch (see section on generating patches).  This is the default'
...
complete -c git-diff -l name-only -d 'Show only names of changed files'
...
complete -c git-diff -o 'l<num>' -d 'The -M and -C options require O(n^2) processing time where n is the number of…'
