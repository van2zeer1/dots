# git-diff-files
# Autogenerated from man page /bedrock/cross/man/man1/git-diff-files.1
complete -c git-diff-files -s p -s u -l patch -d 'Generate patch (see section on generating patches)'
...
complete -c git-diff-files -l name-only -d 'Show only names of changed files'
...
complete -c git-diff-files -o 'l<num>' -d 'The -M and -C options require O(n^2) processing time where n is the number of…'
