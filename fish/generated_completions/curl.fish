# curl
# Autogenerated from man page /bedrock/cross/man/man1/curl.1
complete -c curl -l abstract-unix-socket -d '(HTTP) Connect through an abstract Unix domain socket, instead of using the n…'
complete -c curl -l alt-svc -d '(HTTPS) WARNING: this option is experimental.  Do not use in production'
...
complete -c curl -l disable-epsv -d '(FTP) (FTP) Tell curl to disable the use of the EPSV command when doing passi…'
...
complete -c curl -l doh-cert-status -d '(all) Same as --cert-status but used for DOH (DNS-over-HTTPS).   Added in 7'
complete -c curl -l doh-insecure -d '(all) Same as -k, --insecure but used for DOH (DNS-over-HTTPS).   Added in 7'
...
complete -c curl -l fail-with-body -d '(HTTP)  Return an error on server errors where the HTTP response code is 400 …'
...
complete -c curl -l ftp-create-dirs -d '(FTP SFTP) When an FTP or SFTP URL/operation uses a path that doesn\'t current…'
...
complete -c curl -l hsts -d '(HTTPS) WARNING: this option is experimental.  Do not use in production'
...
complete -c curl -l http2 -d '(HTTP) Tells curl to use HTTP version 2.   See also --http1. 1 and --http3'
...
complete -c curl -l libcurl -d 'Append this option to any ordinary curl command line, and you will get a libc…'
...
complete -c curl -l metalink -d 'This option can tell curl to parse and process a given URI as Metalink file (…'
...
complete -c curl -l netrc-optional -d 'Very similar to -n, --netrc, but this option makes the '
...
complete -c curl -l noproxy -d 'Comma-separated list of hosts which do not use a proxy, if one is specified'
...
complete -c curl -l pass -d '(SSH TLS) Passphrase for the private key  If this option is used several time…'
...
complete -c curl -l proto -d 'Tells curl to limit what protocols it may use in the transfer'
